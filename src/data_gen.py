...
def generate_customers(n_customers: int = 1000, cfg: dict | None = None, seed: int = 42) -> pd.DataFrame:
    ...
    shares = np.array([personas_cfg[p]["share"] for p in personas], dtype=float)
...
def generate_transactions(
    ...
    for _, row in customers.iterrows():
        ...
        for _ in range(n_visits):
            ...
            fuel = max(5.0, np.random.normal(p["fuel_liters_mean"], p["fuel_liters_sd"]))
            coffee = int(random.random() < p["coffee_attach_rate"])
            carwash = int(random.random() < p["carwash_attach_rate"])
            amount = float(p["avg_check"]) * (1.0 + np.random.normal(0.0, 0.10))
    ...
    tx.sort_values(["datetime", "customer_id"], inplace=True, ignore_index=True)
    return tx
...
if __name__ == "__main__":
    ...
    print(f"✅ Сгенерировано {len(customers)} клиентов и {len(tx)} транзакций.")  # Итог
