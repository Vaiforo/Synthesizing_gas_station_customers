...
1. Создаёт синтетических клиентов по 4 портретам (Commuter, Thrifty, Weekend, Shopper и др);  
...
n_users = col1.number_input(
    "Количество клиентов", min_value=100, max_value=10000, value=3000, step=100)
days = col2.number_input("Период генерации (дней)",
                         min_value=30, max_value=180, value=90, step=10)
campaign_name = col3.selectbox("Кампания для симуляции", [
                               "fuel_discount", "coffee_coupon", "без кампании"])

use_mapped = st.checkbox(
    "Использовать маппинг персон (после анализа фичей)", value=False)
