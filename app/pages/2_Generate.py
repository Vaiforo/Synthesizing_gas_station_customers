...
st.set_page_config(page_title="Генерация данных", layout="wide")
...
n_users = col1.number_input(
    "Количество клиентов", min_value=100, max_value=100_000, value=3000, step=100)
days = col2.number_input("Период (дней)", min_value=30,
                         max_value=365, value=90, step=10)
seed = col3.number_input("Seed (для воспроизводимости)",
...
with st.expander("Доли портретов из конфигурации"):
    ...
    if personas:
        pr = pd.DataFrame(
            [{"persona": k, "share": v.get("share", None)}
...
if st.button("Сгенерировать данные", use_container_width=True):
    with st.spinner("Генерируем synthetic customers & transactions..."):
        ensure_dirs()
        customers, transactions = cached_generate(
            int(n_users), int(days), cfg, seed=int(seed))

    st.success(
        f"Готово! Клиенты: {len(customers):,} | Транзакций: {len(transactions):,}".replace(",", " "))
    st.caption(
        f"Файлы будут сохранены по запросу в: `{CUSTOMERS_CSV}` и `{TRANSACTIONS_CSV}`")
    ...
    st.subheader("Скачать/Сохранить")
