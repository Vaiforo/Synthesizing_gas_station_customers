...
st.set_page_config(page_title="🧭 Маппинг персон", page_icon="🧭", layout="wide")

st.title("🧭 Rule-based маппинг пользователей к портретам")
st.markdown(
    """
Этот шаг считает агрегаты поведения по транзакциям и **присваивает каждому клиенту портрет** (Commuter / Thrifty / Weekend / Shopper)
...
if src_choice == "Загрузить transactions.csv":
    ...
    if up is not None:
        ...
        st.success(f"Файл загружен: {up.name}  ·  строк: {len(transactions):,}".replace(",", " "))
else:
    if Path(TRANSACTIONS_CSV).exists():
        transactions = pd.read_csv(TRANSACTIONS_CSV)
        st.info(f"Используем файл: `{TRANSACTIONS_CSV}`  ·  строк: {len(transactions):,}".replace(",", " "))
    else:
        st.error("❌ Не найден data/transactions.csv. Сначала сгенерируйте данные на вкладке **Generate**.")
...
if transactions is not None:
    ...
    if do_map:
        ...
        seg = mapping["assigned_persona"].value_counts().rename_axis("persona").reset_index(name="users")
