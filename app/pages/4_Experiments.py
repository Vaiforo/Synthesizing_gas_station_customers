...
st.set_page_config(page_title="🎯 Эксперименты A/B", page_icon="🎯", layout="wide")
...
campaign_name = col1.selectbox("Кампания", list(cfg.get("campaigns", {}).keys()))
ratio = col2.slider("Доля тестовой группы (B)", 0.1, 0.9, 0.5, 0.05)
seed = col3.number_input("Seed", min_value=0, max_value=999999, value=42, step=1)
...
if st.button("🚀 Запустить эксперимент", use_container_width=True):
    ...
    st.info("Выберите кампанию и нажмите **Запустить эксперимент**, чтобы увидеть эффект A/B.")
